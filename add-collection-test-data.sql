...
BEGIN
    ...
    IF test_restaurant_count = 0 THEN
        ...
        VALUES
            ...
            (gen_random_uuid(), 'REST006', 'Thai Garden', 'Thai', '$$', '987 Cedar Ln', NOW(), NOW()),
            (gen_random_uuid(), 'REST007', 'Indian Spice', 'Indian', '$$', '147 Birch Ave', NOW(), NOW()),
            (gen_random_uuid(), 'REST008', 'French Bistro', 'French', '$$$', '258 Spruce St', NOW(), NOW());
        RAISE NOTICE 'Created 8 test restaurants';
    ELSE
...
BEGIN
    ...
    SELECT array_agg(id) INTO test_restaurant_ids FROM restaurants LIMIT 4;
    ...
    IF test_collection_count = 0 AND test_user_id IS NOT NULL AND test_restaurant_ids IS NOT NULL THEN
        ...
        INSERT INTO collections (id, name, description, created_by, collection_code, is_public, collection_type, restaurant_ids, created_at, updated_at)
        VALUES
            (gen_random_uuid(), 'Business Lunch', 'Professional dining options', test_user_id, 'COL003', false, 'shared', test_restaurant_ids, NOW(), NOW())
        RETURNING id INTO test_collection_id;
...
BEGIN
    ...
    SELECT id INTO test_user_id FROM users ORDER BY created_at DESC LIMIT 1;
    SELECT id INTO test_collection_id FROM collections WHERE collection_type = 'shared' ORDER BY created_at DESC LIMIT 1;
...
-- Test the getUserPlans function with the new data
...
SELECT
  ...
  CASE
    ...
    THEN 'TEST DATA CREATED SUCCESSFULLY - USER SHOULD NOW SEE COLLECTIONS WITH DATA'
    WHEN (SELECT COUNT(*) FROM users) > 0
    AND (SELECT COUNT(*) FROM collections) > 0
    AND (SELECT COUNT(*) FROM restaurants) > 0
    THEN 'USERS, COLLECTIONS, AND RESTAURANTS EXIST - CHECK MEMBERSHIPS AND ACTIVITY'
    WHEN (SELECT COUNT(*) FROM users) > 0
    AND (SELECT COUNT(*) FROM collections) > 0
