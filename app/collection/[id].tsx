...
function InsightsTab({ collection, rankedRestaurants, discussions, collectionMembers, styles, setShowCommentModal }: InsightsTabProps) {
  return (
    <View style={styles.insightsContainer}>
      ...
      <View style={styles.insightsSection}>
        <Text style={styles.insightsTitle}>👥 Member Voting Patterns</Text>
        <View style={styles.insightsGrid}>
          {(() => {
            // Get all unique members who have voted
            const allVoters = new Set<string>();
            rankedRestaurants.forEach(({ meta }) => {
              meta.voteDetails?.likeVoters?.forEach((voter: any) => allVoters.add(voter.userId));
              meta.voteDetails?.dislikeVoters?.forEach((voter: any) => allVoters.add(voter.userId));
            });

            // Create a map of user voting patterns
            const userVotingPatterns = Array.from(allVoters).map(userId => {
              const userLikes: any[] = [];
              const userDislikes: any[] = [];

              rankedRestaurants.forEach(({ restaurant, meta }) => {
                const likeVote = meta.voteDetails?.likeVoters?.find((v: any) => v.userId === userId);
                const dislikeVote = meta.voteDetails?.dislikeVoters?.find((v: any) => v.userId === userId);
                
                if (likeVote) {
                  userLikes.push({ restaurant, voter: likeVote });
                } else if (dislikeVote) {
                  userDislikes.push({ restaurant, voter: dislikeVote });
                }
              });

              // Get user name from first vote
              const firstVote = [...userLikes, ...userDislikes][0];
              const userName = firstVote?.voter?.name || 'Unknown User';
              ...
              return {
                ...
                totalVotes: userLikes.length + userDislikes.length
              };
            }).filter(user => user.totalVotes > 0)
              .sort((a, b) => b.totalVotes - a.totalVotes);

            return userVotingPatterns.map((user, index) => (
              <View key={user.userId} style={styles.insightsContent}>
                <View style={styles.userVotingHeader}>
                  <View style={styles.userVotingAvatar}>
                    <Text style={styles.userVotingInitial}>
                      {user.userName?.split(' ')[0]?.charAt(0).toUpperCase() || 'U'}
                    </Text>
                  </View>
                  <View style={styles.userVotingInfo}>
                    <Text style={styles.userVotingName}>{user.userName}</Text>
                    <Text style={styles.userVotingStats}>
                      {user.totalVotes} votes • {user.likes.length} likes • {user.dislikes.length} dislikes
                    </Text>
                  </View>
                </View>
                ...
                {user.likes.length > 0 && (
                  <View style={styles.userLikesSection}>
                    <Text style={styles.userLikesTitle}>👍 Liked:</Text>
                    {user.likes.map(({ restaurant }, idx) => (
                      <View key={`${user.userId}-like-${restaurant.id}-${idx}`} style={styles.userRestaurantItem}>
                        <View style={styles.userRestaurantImage}>
                          <Text style={styles.userRestaurantInitial}>
                            {restaurant.name?.charAt(0).toUpperCase() || 'R'}
                          </Text>
                        </View>
                        <View style={styles.userRestaurantInfo}>
                          <Text style={styles.userRestaurantName}>{restaurant.name}</Text>
                          <Text style={styles.userRestaurantCuisine}>{restaurant.cuisine || 'Restaurant'}</Text>
                        </View>
                        <ThumbsUp size={16} color="#10B981" />
                      </View>
                    ))}
                ...
                {/* User's Disliked Restaurants */}
                {user.dislikes.length > 0 && (
                  <View style={styles.userDislikesSection}>
                    <Text style={styles.userDislikesTitle}>👎 Disliked:</Text>
                    {user.dislikes.map(({ restaurant }, idx) => (
                      <View key={`${user.userId}-dislike-${restaurant.id}-${idx}`} style={styles.userRestaurantItem}>
                        <View style={styles.userRestaurantImage}>
                          <Text style={styles.userRestaurantInitial}>
                            {restaurant.name?.charAt(0).toUpperCase() || 'R'}
                          </Text>
                ...
                {user.likes.length === 0 && user.dislikes.length === 0 && (
                  <Text style={styles.noUserVotes}>No votes recorded</Text>
                )}
              </View>
            ));
          })()}
